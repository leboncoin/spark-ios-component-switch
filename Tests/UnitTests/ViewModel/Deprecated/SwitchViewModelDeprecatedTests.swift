...
private extension SwitchViewModelDeprecatedTests {
    ...
    func testGetColorsUseCaseMock(
        ...
        XCTAssertEqual(stub.getColorsUseCaseMock.executeWithThemeCallsCount,
                       numberOfCalls,
                       "Wrong call number on execute on getColorsUseCase")

        if numberOfCalls > 0, let givenTheme, let givenIntent {
            let receivedTheme = stub.getColorsUseCaseMock.executeWithThemeReceivedTheme
            XCTAssertIdentical(try XCTUnwrap(receivedTheme?.colors as? ColorsGeneratedMock),
...
private final class Stub {
    ...
    init(
        ...
        // Dependencies
        let getColorsUseCaseMock = SwitchGetColorsUseCaseableGeneratedMock()
        getColorsUseCaseMock.executeWithThemeReturnValue = self.colorsMock
